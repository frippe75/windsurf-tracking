        ...
        >
          ...
          <SelectContent>
            {backends.map((backend) => (
              <SelectItem key={backend.id} value={backend.id} className="[&>span:first-child]:hidden">
                {/* Absolute indicator in the reserved gutter (does not affect text alignment) */}
                {selectedBackend?.id === backend.id && (
                  <>
                    {backendStatus === "healthy" && (
                      <span className="absolute left-2 top-1 h-2.5 w-2.5">
                ...
                {/* Text: perfectly left-aligned due to SelectItem pl-8 */}
