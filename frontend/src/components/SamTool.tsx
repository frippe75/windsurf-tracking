...
/**
 * SAM3 "Detect" tool — the active-tool options for the left rail (rendered when the Detect
 * tool is selected). SAM3 is the concept/track engine, routed BY CAPABILITY (no model picker);
 * SAM2 stays the click-refine engine elsewhere.
 *
 *  - Detect on frame: text prompt -> /pipeline/segment (concept-segment) -> review -> Add objects
 *  - Track N frames:  onTrack() -> SAM3 video predictor across a window -> tracked annotations
 *
 * The frame/window are extracted SERVER-SIDE (the app's <video> is a cross-origin blob: URL),
 * so we only send the video id + timestamp; the pipeline service resolves + extracts.
 */
...
type Props = {
  ...
  // (text, windowFrames, onProgress) -> number of annotations created
  onTrack: (text: string, windowFrames: number, onProgress: (s: string) => void) => Promise<number>;
  selectedClassId?: string;
  videoReady: boolean;
};

export function SamTool({ onAddDetections, onTrack, selectedClassId, videoReady }: Props) {
  const [caps, setCaps] = useState<string[]>([]);
  const [prompt, setPrompt] = useState(() => localStorage.getItem("samPrompt") || "windsurf sail rig");
  ...
  useEffect(() => localStorage.setItem("samPrompt", prompt), [prompt]);
  ...
  const ready = videoReady && !!selectedClassId;
  const gate = useMemo(() => {
    if (!videoReady) return "Load a video to detect.";
    if (!selectedClassId) return "Select a class to detect into.";
    return "";
  }, [videoReady, selectedClassId]);
  ...
  async function track() {
    ...
    try {
      await onTrack(prompt, trackWindow, setTrackStatus);
    } catch (e: any) {
        ...
        {hasConcept && (
          <>
            <div className="space-y-1">
              <label className="text-[11px] text-muted-foreground">Concept prompt</label>
              <Input value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="e.g. windsurf sail rig" className="h-8 text-xs" />
            </div>
            ...
            <Button onClick={detect} disabled={!ready || busy} size="sm" className="w-full">
              {busy ? (<><Loader2 className="h-3.5 w-3.5 mr-1 animate-spin" />Detecting… (cold start ~1–4 min)</>) : "Detect on frame"}
            </Button>

            {foundCount !== null && (
              <div className="text-[11px] text-muted-foreground">
                {foundCount} found · {kept.length} ≥ {minScore.toFixed(2)}
              </div>
            )}
            {kept.length > 0 && (
            ...
            {hasTrack && (
              <div className="pt-2 border-t border-border space-y-2">
                ...
                <Button onClick={track} disabled={!ready || tracking} size="sm" className="w-full">
                  {tracking ? (<><Loader2 className="h-3.5 w-3.5 mr-1 animate-spin" />{trackStatus || "Tracking…"}</>) : `Track across ${trackWindow} frames`}
                </Button>
                {tracking && (
                ...
                <div className="text-[10px] text-muted-foreground">SAM3-native · tracks from the current frame</div>
              </div>
            )}
          </>
        )}
